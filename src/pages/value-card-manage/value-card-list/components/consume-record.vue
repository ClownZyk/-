    ...
    <el-form class="filter" ref="filterParamRef" :model="filterParam" inline>
      <div class="filter-left">
        ...
        <el-form-item label="消费类型" prop="type">
          <el-select v-model="filterParam.type" placeholder="请选择消费类型"> <el-option v-for="item in typeList" :key="item.value" :label="item.label" :value="item.value"> </el-option> </el-select>
        </el-form-item>
        ...
        <el-form-item> <el-button type="primary" @click="getConsumeList('refresh')">搜索</el-button> </el-form-item>
      </div>
      <div class="filter-right">
        <el-form-item> <el-button :loading="exportLoading" type="warning" @click="recordExport" :disabled="consumeList.length <= 0">导出</el-button> </el-form-item>
      </div>
    </el-form>
    <div class="table-list">
      <el-table v-loading="tableLoading" :data="consumeList" border>
        <el-table-column label="卡编号" prop="title" width="120" align="center"></el-table-column>
        <el-table-column label="消费订单号" min-width="180" align="center" show-overflow-tooltip>
        ...
        <el-table-column label="手机号" min-width="120" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <div>{{ scope.row.phone || '--' }}</div>
          </template>
        </el-table-column>
        <el-table-column label="消费金额(元)" prop="amount" min-width="140" align="center" show-overflow-tooltip> </el-table-column>
        <el-table-column label="卡余额(元)" prop="surplusAmount" min-width="140" align="center" show-overflow-tooltip> </el-table-column>
        <el-table-column label="消费途径" prop="source" min-width="160" align="center" show-overflow-tooltip> </el-table-column>
        <el-table-column label="消费时间" prop="createTime" width="180" align="center" show-overflow-tooltip> </el-table-column>
      </el-table>
      <el-pagination
...
export default {
  data() {
    return {
      ...
      pageSize: 15,
      total: 0,
      typeList: [{ value: 2, label: '支出' }, { value: 3, label: '退款' }]
    }
  },
  mounted() {
    // this.getCardType()
