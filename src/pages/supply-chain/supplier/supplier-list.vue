    ...
    <div class="supplier-list" v-if="containerIsShow">
      ...
      <el-form class="filters" ref="searchForm" :model="search" :inline="true">
        <div class="filters-left">
          <el-form-item>
            <el-input v-model="search.name" placeholder="请输入供应商检索" class="filters-search" @keyup.enter.native="GetListByPage"></el-input>
      ...
      <div class="list-table">
        <el-table class="table-main" ref="listTable" :data="list" border tooltip-effect="dark" @selection-change="selectTableRow" v-loading="listLoading">
          ...
          <el-table-column prop="sname" label="供应商名称" min-width="180" show-overflow-tooltip></el-table-column>
          <el-table-column align="center" prop="belongsBank" label="所属银行" width="200" show-overflow-tooltip></el-table-column>
          <el-table-column align="center" prop="bankAccount" label="银行账号" width="200" show-overflow-tooltip></el-table-column>
          <el-table-column align="center" prop="ghtype" label="供货类型" width="150" show-overflow-tooltip>
            <template slot-scope="scope">
              {{scope.row.ghtype === 1 ? '批发销售' : ''}}
            </template>
          </el-table-column>
          <el-table-column align="center" prop="ghRange" label="供货范围" width="180" show-overflow-tooltip></el-table-column>
...
import {SupplierGetListByPage, SupplierOperation} from '@/api/system'
export default {
  name: 'supply-supplier-list',
  watch: {
    '$route' (to, from) {
      if (to.name === 'supply-supplier-list') {
        this.GetListByPage()
  ...
  methods: {
    ...
    operateFunc (row, title, action) {
      ...
      switch (title) {
        case '添加':
          this.$router.push({path: '/supply-supplier-detail', query: {operate: 'add'}})
          break
        case '编辑': // 编辑
          this.$router.push({path: '/supply-supplier-detail', query: {id: row._id}})
          break
        case '删除': // 删除
          ...
          break
        case '查看': // 查看
          this.$router.push({path: '/supply-supplier-detail', query: {id: row._id, operate: 'view'}})
          break
        default:
          return false
      }
    ...
    async GetListByPage () {
      ...
      await SupplierGetListByPage(params).then(res => {
