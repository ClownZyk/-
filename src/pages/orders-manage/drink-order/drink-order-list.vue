...
import { getDrinkOrderList, drinkOrderExport, getAllStore } from '@/api/system'

export default {
  mixins: [Filters],
  watch: {
    $route(to, from) {
      if (to.name === 'drink-order-list') {
        this.initData()
  ...
  methods: {
    ...
    openPrint(row) {
      this.printShow = true
      this.$refs.printRef.getPrintList(3, row)
    },
    closePrint(val) {
      this.printShow = val
    },
