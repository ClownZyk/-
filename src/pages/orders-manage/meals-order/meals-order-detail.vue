  ...
  <div class="detail-container">
    ...
    <div class="detail-item" v-show="orderDetail.product && orderDetail.product.length > 0">
      ...
      <el-table :data="orderDetail.product" border style="width: 100%" :summary-method="getCommonSummaries" show-summary>
        <el-table-column prop="sku.barCode" label="商品条码" min-width="180" align="center" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="productName" label="商品名称" min-width="100" align="center" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column sortable label="实付总价" min-width="100" align="center" show-overflow-tooltip>
          <template slot-scope="scope">{{ Math.round(scope.row.sku.count * scope.row.sku.memberPrice * 100) / 100 }}</template>
        </el-table-column>
      </el-table>
    </div>
    <!-- 促销商品 -->
...
<script>
import { getMakeOrderDetail, updateMakeOrderStatus } from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    getOrderDetail() {
      ...
      getMakeOrderDetail(params).then(
        res => {
          if (res.success) {
    ...
    orderStateChange() {
      this.$confirm('是否修改订单状态', '提示', {
        ...
        .then(() => {
          ...
          updateMakeOrderStatus(params).then(
            res => {
              if (res.success) {
                this.$router.go(-1)
              }
    ...
    getCommonSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (index === 6) {
          ...
          let total = 0
          data.forEach(item => {
            total += Math.round(item.sku.count * item.sku.memberPrice * 100) / 100
          })
