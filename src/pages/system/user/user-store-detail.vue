<template>
  <div class="page-contain">
    ...
    <el-form size="small" ref="shopDetail" :model="shopDetail" label-width="80px" label-position="top" class="form-contain" :rules="shopDetailRules">
      <el-row :gutter="20">
        <el-col :span="12">
          <el-divider content-position="left">店铺基本信息配置</el-divider>
          <!-- 门店名称 -->
          <el-row>
    ...
    initMap() {
      this.deliverMap = new window.TMap.Map(this.$refs.mapContainer, {
        center: new window.TMap.LatLng(
          this.shopDetail.location[0] ? this.shopDetail.location[0].lat : this.shopDetail.coordinate[1] ? this.shopDetail.coordinate[1] : 39.90736606309809,
          this.shopDetail.location[0] ? this.shopDetail.location[0].lng : this.shopDetail.coordinate[0] ? this.shopDetail.coordinate[0] : 116.39825820922852
        ),
        zoom: 16,
        viewMode: '2D',
        baseMap: 'vector'
      })
      this.initEditor(this.shopDetail.coordinate, this.shopDetail.location)
    },
      ...
      GetSetUpObj(parmas).then(
        res => {
          if (res.success) {
            ...
            if (refresh !== 'refresh') {
              this.initMap()
              this.deliverMap.setCenter(
                new window.TMap.LatLng(
                  res.data.location[0] ? res.data.location[0].lat : this.shopDetail.coordinate[1] ? this.shopDetail.coordinate[1] : 39.90736606309809,
                  res.data.location[0] ? res.data.location[0].lng : this.shopDetail.coordinate[0] ? this.shopDetail.coordinate[0] : 116.39825820922852
                )
              )
            }
