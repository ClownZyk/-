...
import {GetV8CategoryInfo, Operation, SynchronismCategory, GetProductInfoByCategory} from '@/api/system'
  ...
  methods: {
    ...
    handleAsyncProduct () {
      ...
      if (this.checkedDatas.length > 0) {
        this.$confirm('确认同步已选中的数据吗？', '提示', {}).then(() => {
          let params = {
          ...
          GetProductInfoByCategory(params).then(res => {
            // console.log(res)
            if (res.success) {
