...
<script>
import {SeckillOperation, SeckillGetDetailById} from '@/api/system'
  ...
  methods: {
    ...
    async init () {
      if (this.$route.query.id !== undefined) {
        ...
        await SeckillGetDetailById(parmas).then(res => {
            if (res.success) {
              this.form = res.data
              // 处理时间数组
              let tempArr = []
              if (this.form.seckillStartTime && this.form.seckillEndTime) {
                tempArr.push(new Date(this.form.seckillStartTime))
                tempArr.push(new Date(this.form.seckillEndTime))
                this.$set(this.form, 'seckillTimeArr', tempArr)
              }
              this.form.selectedGoodsName = this.form.prod.productName
