      ...
      <el-form :model="detail" :rules="rules" class="msg-dialog" ref="detail">
        ...
        <div class="mag-dialog-row">
          ...
          <el-form-item prop="totalPrice">
            <div class="form-item-title">产品总价<span class="border"></span> <span class="EnName">oriPrice</span></div>
            <el-input-number :min="0" controls-position="right" class="mag-input" v-model="detail.totalPrice" :disabled="true"></el-input-number>
          </el-form-item>
          <el-form-item>
        ...
        <div class="mag-dialog-row">
           <el-form-item>
            <div class="form-item-title">套餐产品<span class="border"></span> <span class="EnName">condition</span>
              <el-button type="primary" size="mini" style="margin-left: 160px;"  @click="handleProductSelect(true)">添加</el-button>
            </div>
            <div class="discount-list">
               <el-table :data="detail.setMealProduct" border>
        ...
        <div class="mag-dialog-row">
          <el-form-item>
            ...
            <my-upload :imgs="detail.prodImage"
                       :limitUploadImgs="3"
                       ref="uploadImg"
...
<script>
  import {SetMealOperation, SetMealDetails} from '@/api/system'
  ...
  import productSelectRadio from '@/components/product-select-radio'
    export default {
        ...
        return {
          ...
          rules: {
            setMealName: [
              ...
              { min: 1, max: 10, message: '不得超过10个字符', trigger: 'blur' }
            ],
            setPrice: [
      ...
      watch: {
        'detail.setMealProduct': {
          handler(v) {
            console.log(v)
            if (v.length > 0) {
              this.detail.totalPrice = v.reduce((preTotal, item) => {
      ...
      methods: {
        _detail() {
          ...
          SetMealDetails(params).then(res => {
              if (res.success) {
        ...
        saveSelected (selectRow) {
          if (this.detail.setMealProduct.length > 0) {
            let _skuId = this.detail.setMealProduct.filter(item => item.barCode === selectRow.sku[0].barCode)
            if (_skuId.length > 0) {
          ...
          this.detail.setMealProduct.push(productDetail)
        },
        // 增删改
        _submit(formName) {
         ...
         _delect(index) {
          this.detail.setMealProduct.splice(index, 1)
        },
        // 增删改
        _option(action) {
          if (this.detail.setMealProduct.length <= 0) {
          ...
          this.detail.setMealProduct = this.detail.setMealProduct.map((item, index) => {
            return {
              no: index + 1,
              ...item
            }
          ...
          SetMealOperation(params).then(res => {
              if (res.success) {
