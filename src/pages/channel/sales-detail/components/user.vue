<template>
  <div class="user">
    <el-table class="main-table" ref="mainTable" :data="personList" border tooltip-effect="dark" v-loading="listLoading">
      ...
      <el-table-column align="center" prop="phone" label="手机" width="250" show-overflow-tooltip></el-table-column>
      <el-table-column align="center" prop="email" label="邮箱" width="250" show-overflow-tooltip></el-table-column>
      <el-table-column align="center" label="是否会员" width="250" show-overflow-tooltip>
        <template slot-scope="scope">
          {{scope.row.vipCard.isVip ? '是' : '否'}}
        </template>
      </el-table-column>
...
<script>
import {UserCRMList, UserCRMOperation} from '@/api/system'
export default {
  name: 'sales-user',
  data () {
    return {
  ...
  methods: {
    ...
    operateFunc (row, title, action) {
      if (action === 1) {
        this.$router.push({path: '/scrm-detail', query: {id: row._id, openid: row.openid}})
      } else if (action === 2) {
        this.$confirm('确认删除吗?', '提示', {
          ...
          UserCRMOperation(parmas).then(res => {
            this.$message({
              message: res.msg,
    ...
    async GetListByPage () {
      if (this.$route.query.id !== undefined) {
        let parmas = {
          parentOpenid: this.$route.query.id,
          nickName: '',
        ...
        await UserCRMList(parmas).then(res => {
          if (res.success) {
