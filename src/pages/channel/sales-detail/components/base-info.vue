      ...
      <el-form ref="form" :model="form" label-width="80px" label-position="top"  class="form-contain" :rules="rules">
        ...
        <div class="form-contain-item">
          ...
          <el-form-item prop="channelId">
            <div class="form-item-title">所属渠道<span class="border"></span><span class="EnName">CHANNEL</span></div>
            <el-select v-model="form.channelId" placeholder="请选择所属渠道">
              <el-option v-for="(item, index) in channelList" :key="index" :value="item.value" :label="item.label">{{item.label}}</el-option>
            </el-select>
          </el-form-item>
        </div>
        <div class="form-contain-item">
          ...
          <el-form-item>
            <div class="form-item-title">邀请用户的二维码链接<span class="border"></span><span class="EnName">QRCODE</span></div>
            <el-input v-model="form.qrCode" placeholder="请输入邀请码" style="width: 420px;"></el-input>
          </el-form-item>
...
<script>
import {SalesManOperation, SalesManGetDetailById, ChannelGetDropDownList} from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    async ChannelGetDropDownList () {
      ...
      await ChannelGetDropDownList(parmas).then(res => {
        if (res.success) {
    ...
    async init () {
      ...
      if (this.$route.query.id !== undefined) {
        ...
        this.loading = true
        await SalesManGetDetailById(parmas).then(res => {
            if (res.success) {
              this.loading = false
              this.form = res.data
              // 拼图片的回显列表
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          type: 'warning'
            ...
            SalesManOperation(params).then(res => {
              this.$message({
                  message: res.msg,
