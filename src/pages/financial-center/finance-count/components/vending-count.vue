  ...
  <div class="contain-main">
    <el-form class="filter" ref="form" :model="searchForm" label-width="80px" :inline="true">
      ...
      <div class="filter-bottom">
        <div class="filter-bottom-left">
          ...
          <el-form-item>
            <el-select v-model="searchForm.vmcode" placeholder="请选择售卖机" clearable @change="machineChange" style="width: 260px">
              <el-option v-for="item in vendingMachineList" :key="item._id" :label="item.name" :value="item.vUnionCode"> </el-option>
            </el-select>
          </el-form-item>
    ...
    <div class="table-list">
      <el-table v-loading="tableLoading" :data="listData" border @sort-change="sortChange">
        ...
        <el-table-column prop="vmCode" label="售卖机编码" min-width="160" show-overflow-tooltip></el-table-column>
      </el-table>
      <el-pagination
  ...
  data() {
    return {
      searchForm: {
        isFixedSearch: true,
        time: [],
        stage: 1, // 1 当日 2 当周 3 当月 4 自定义时间
        vmcode: '',
        prodName: '',
        isAsc: true,
  ...
  methods: {
    ...
    machineChange(val) {
      if (this.vendingMachineList.length > 0) {
        let machineInfo = this.vendingMachineList.find(item => item.vUnionCode === val)
