<template>
  <div>
    <div class="gem-back-header"><el-page-header @back="goBack" content="支付分退款订单"> </el-page-header></div>
    <el-form :model="searchForm" inline>
      <el-form-item label="商户服务订单号"> <el-input type="text" v-model.trim="searchForm.oriOrderid" placeholder="请输入商户服务订单号" clearable></el-input> </el-form-item>
    ...
    <div class="table-list">
      <el-table v-loading="tableLoading" :data="refundOrderList" border>
        ...
        <el-table-column label="原商户服务订单号" prop="orderId" min-width="200" align="center" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column label="操作" width="160" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button size="mini" type="warning" @click="orderRefund(scope.row)" :disabled="scope.row.state === '已同意退款' || scope.row.state === '不同意退款'">退款</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
<script>
import { RefoundGetListByPage, refundPayScoreOrder, refundAlipayScoreOrder } from '@/api/system'
      ...
      this.$confirm('确认执行退款操作?', '提示', {
        ...
        .then(() => {
          if (this.$route.query.refundType === 1) {
            this.wxRefundOperation(row)
          } else if (this.$route.query.refundType === 2) {
            this.aliRefundOperation(row)
          }
        })
        .catch(() => {})
    },
    wxRefundOperation(row) {
      let params = {
        transaction_id: row.transactionId,
        out_refund_no: row.out_refund_no,
        total: row.total,
        refund: row.refund_fee,
        _id: row._id
      }
      // console.log(params)
      refundPayScoreOrder(params).then(res => {
        this.$message.success(res.msg)
        this.getRefundOrderList()
      })
    },
    aliRefundOperation(row) {
      let params = {
        orderId: row.orderId,
        refund: row.refund_fee,
        _id: row._id
      ...
      // console.log(params)
      refundAlipayScoreOrder(params).then(res => {
        this.$message.success(res.msg)
        this.getRefundOrderList()
      })
    },
    goBack() {
      this.$router.push({ path: '/pay-score-order' })
    },
    searchCondition() {
      this.pageIndex = 1
      this.getRefundOrderList()
