    ...
    <div class="product-list" v-if="containerIsShow">
      ...
      <el-form class="filter" :model="filterParam" ref="filterParamRef" :inline="true">
        ...
        <div class="filter-right">
          <el-form-item v-if="isManage"> <el-button type="primary" @click="gotoFormulaDetail">添加自制饮品</el-button> </el-form-item>
          <el-form-item v-else> <el-button :loading="asyncLoading" type="warning" @click="handleDrinkProductAsync">同步自制饮品</el-button> </el-form-item>
        </div>
      </el-form>
        ...
        <el-table v-loading="tableLoading" :data="productList" border style="width: 100%">
          <el-table-column type="expand">
            <template slot-scope="scope">
              <el-table :data="scope.row.sku" border style="width: 50%;margin: 10px 0 10px 10px" size="small">
      ...
      <!-- 饮品商品选择器 -->
      <drink-product-select ref="drinkRef" :isSync="true" :isDrinkProductShow="isDrinkProductShow" @savedDrinkData="saveData" @closedDrinkDialog="closedDrinkDialog"></drink-product-select>
    </div>
...
<script>
import { GetAllCategory, getDrinkZoneList, getDrinkProduct, drinkProductOperation, drinkRawProductAsync } from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    saveData(newSelectedList) {
      ...
      this.drinkProductAsync(uniqueCodeIds)
    },
    // 自制产品同步
    drinkProductAsync(uniqueCodeIds) {
      let params = {
      ...
      drinkRawProductAsync(params).then(
        res => {
