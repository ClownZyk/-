...
<script>
import { getDrinkRawProductBatch, drinkRawProductBatchOperation, drinkRawProductBatchScrap } from '@/api/system.js'
...
export default {
  ...
  methods: {
    ...
    getSalesBatch() {
      ...
      getDrinkRawProductBatch(parmas).then(
        res => {
          if (res.success) {
    ...
    editBatch() {
      this.$confirm('确认编辑？', '提示', {
        ...
        drinkRawProductBatchOperation(parmas).then(
          res => {
            if (res.success) {
    ...
    toLoss(row) {
      this.$confirm('确认将该批次报损么', '提示', {
        ...
        .then(() => {
          ...
          drinkRawProductBatchScrap(parmas).then(
            res => {
              if (res.success) {
                this.$message.success(res.msg)
                this.getSalesBatch()
