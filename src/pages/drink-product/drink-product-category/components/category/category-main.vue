<template>
  <div class="list-container">
    ...
    <el-form class="filters">
      <el-form-item>
        <el-input v-model="search" placeholder="请输入检索内容" class="filters-search" @keyup.enter.native="GetListByPage" clearable></el-input>
        <el-button type="primary" @click="GetListByPage">搜索</el-button>
      </el-form-item>
      <el-form-item>
        <el-button v-if="isManage" type="primary" @click="operateForm('', '添加', 'Add')" v-permission="['/drink-product-category:insert']">新增</el-button>
    ...
    <div class="tree-table" v-loading="listLoading">
      <ul class="tree-table-ul">
        ...
        <li v-for="(item, index) in datas" :key="index">
          <div class="table-tr">
            <div class="select"><el-checkbox @change="clickCheck(null, item)" v-cloak :indeterminate="item.isIndeterminate" v-model="item.checked"></el-checkbox></div>
            ...
            <div class="category-name" :class="{ active: item.isActive, hasChild: item.children && item.children.length }" @click="fold(item, datas)">
              <i class="iconfont icon-lanmuliebie" style="margin-right: 5px;"></i> {{ item.categoryName }}
            </div>
            <div class="order-num"><el-input-number size="small" v-model="item.sortNo" :min="99" :step="1" step-strictly :controls="false" @change="updateSortNo($event, item)"></el-input-number></div>
            <div class="permission"><el-switch v-model="item.isShow" @change="updateIsShow($event, item)"></el-switch></div>
            <div class="operate">
              <el-button type="warning" size="mini" @click="operateForm(item, '编辑', 'Update')" v-permission="['/drink-product-category:update']">编辑</el-button>
              <!--
                <el-button type="primary" size="mini" @click="operateForm(item._id, '添加', 'AddSon')"
                  >添加子类</el-button
                >
              -->
              <el-button v-if="isManage" type="danger" size="mini" @click="operateDelete(item)" v-permission="['/drink-product-category:delete']">删除</el-button>
            </div>
          </div>
          <!-- 多级子分类 -->
...
import {GetListByPage, Operation} from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    async GetListByPage () {
      ...
      await GetListByPage(params).then(res => {
