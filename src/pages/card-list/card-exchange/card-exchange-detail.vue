...
import { GiftCardDetail, GiftCardOperation, GetListByPage } from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.includeProd.length <= 0 && this.includeCategory.length <= 0) {
        this.$message.warning('请添加兑换的产品分类或产品！')
        return false
      }
        ...
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {
            ...
            GiftCardOperation(params).then(res => {
              ...
              this.$router.push('/card-exchange')
            })
          })
        }
      })
