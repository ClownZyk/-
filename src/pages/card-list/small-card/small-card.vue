...
<script>
import { smallCardList, smallCardOperation } from '@/api/system'
export default {
  watch: {
    $route(to, from) {
      if (to.name === 'small-card') {
        this._initData()
  ...
  methods: {
    ...
    operateFunc(row, title, action) {
      ...
      switch (title) {
        case '添加':
          this.$router.push('/small-card-detail')
          break
        case '编辑': // 编辑
          this.$router.push({ path: '/small-card-detail', query: { id: row._id } })
          break
        case '删除': // 删除
