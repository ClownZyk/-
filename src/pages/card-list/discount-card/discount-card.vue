...
<script>
import { DiscountCardList, DiscountCardOperation } from '@/api/system'
export default {
  watch: {
    $route(to, from) {
      if (to.name === 'discount-card') {
        this._initData()
  ...
  methods: {
    ...
    operateFunc(row, title, action) {
      ...
      switch (title) {
        case '添加':
          this.$router.push('/discount-card-detail')
          break
        case '编辑': // 编辑
          this.$router.push({ path: '/discount-card-detail', query: { id: row._id } })
          break
        case '删除': // 删除
