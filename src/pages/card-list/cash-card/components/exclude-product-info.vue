<template>
  <div class="product-container">
    <div class="product">
      ...
      <el-form inline style="margin-bottom:20px;">
        ...
        <el-form-item v-if="useRange.includes(2)" label="餐食商品分类：">
          <el-select multiple v-model="selectedMealCategory" placeholder="请选择餐食产品分类" @change="handleMealCategoryChange" @remove-tag="handleCategoryRemove" style="width: 300px">
            <el-option v-for="item in mealCategoryList" :key="item.unionCode" :label="item.name" :value="item.unionCode"> </el-option>
          </el-select>
        </el-form-item>
        <el-form-item v-if="useRange.includes(3)" label="饮品商品分类：">
          <el-select multiple v-model="selectedDrinkCategory" placeholder="请选择饮品产品分类" @change="handleDrinkCategoryChange" @remove-tag="handleCategoryRemove" style="width: 300px">
            <el-option v-for="item in drinkCategoryList" :key="item.unionCode" :label="item.name" :value="item.unionCode"> </el-option>
          </el-select>
        </el-form-item>
      </el-form>
      ...
      <el-form ref="searchForm" :inline="true">
        <el-form-item v-if="useRange.includes(1)"> <el-button type="primary" @click="handleProductSelect(true)">添加零售产品</el-button> </el-form-item>
        <el-form-item v-if="useRange.includes(2)"> <el-button type="warning" @click="addMealProduct">添加餐食产品</el-button> </el-form-item>
        <el-form-item v-if="useRange.includes(3)"> <el-button type="success" @click="addDrinkProduct">添加饮品产品</el-button> </el-form-item>
      ...
      <div class="list-table">
        <el-table class="table-main" ref="listTable" :data="excludeProd" border tooltip-effect="dark" style="width:60%">
    ...
    <!-- 餐食商品选择器 -->
    <meal-product-select ref="mealRef" :isMealProductShow="isMealProductShow" @savedMealData="saveData" @closedMealDialog="closedMealDialog"></meal-product-select>
    <!-- 饮品商品选择器 -->
...
export default {
  ...
  methods: {
    ...
    handleEmitChange(list) {
      ...
      this.$emit('update:excludeCategory', this.selectedCategory)
    },
    ...
    saveData(newSelectedList) {
      ...
      this.$emit('update:excludeProd', this.selectedProducts)
    },
    // 删除
    delFunc (row) {
