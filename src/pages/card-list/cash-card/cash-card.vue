    ...
    <div v-if="containerIsShow">
      ...
      <div class="list-table">
        <el-table class="table-main" :data="contentList" border tooltip-effect="dark" v-loading="listLoading">
          ...
          <el-table-column prop="title" label="标题" align="center" min-width="250" show-overflow-tooltip></el-table-column>
          <el-table-column label="子标题" align="center" min-width="200" show-overflow-tooltip>
...
<script>
import { CashCardList, CashCardOperation } from '@/api/system'
export default {
  watch: {
    $route(to, from) {
      if (to.name === 'cash-card') {
        this._initData()
  ...
  methods: {
    ...
    operateFunc(row, title, action) {
      ...
      switch (title) {
        case '添加':
          this.$router.push('/cash-card-detail')
          break
        case '编辑': // 编辑
          this.$router.push({ path: '/cash-card-detail', query: { id: row._id } })
          break
        case '删除': // 删除
    ...
    operateAxios(item) {
      ...
      CashCardOperation(params).then(res => {
        this.$message({
          message: res.msg,
