<template>
  ...
  <el-dialog :visible.sync="isShow" title="商品选择器 (点击行选择商品)" width="1000px" @closed="closed">
    ...
    <el-table class="table-main" ref="listTable" :data="productList" border v-loading="listLoading">
      ...
      <el-table-column type="expand" prop="sku" label="SKU" width="80">
        <template slot-scope="scope">
          <el-table :data="scope.row.sku" border tooltip-effect="dark" highlight-current-row @current-change="selectTableRow($event, scope.row)">
            <el-table-column prop="title" label="型号" min-width="100"></el-table-column>
            <el-table-column prop="barCode" label="产品条码" min-width="100"></el-table-column>
            <el-table-column prop="stock" label="库存" min-width="100"></el-table-column>
            <el-table-column prop="costPrice" label="供货价" min-width="100"></el-table-column>
            <el-table-column prop="referPrice" label="市场参考价" min-width="100"></el-table-column>
          </el-table>
        </template>
      </el-table-column>
