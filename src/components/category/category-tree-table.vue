  ...
  <div>
    <ul v-show="item.children && item.folded">
      <li v-for="(child, childIndex) in item.children" :key="childIndex">
        <div class="table-tr">
          <div class="select"><el-checkbox :indeterminate="child.isIndeterminate" v-model="child.checked" @change="clickCheck(null, item, child)"></el-checkbox></div>
          ...
          <div class="order-num"><el-input-number size="small" v-model="child.sortNo" :min="99" :step="1" step-strictly :controls="false" @change="updateSortNo($event, child)"></el-input-number></div>
          <div class="permission"><el-switch v-model="child.isShow" @change="updateIsShow($event, child)"></el-switch></div>
          <div class="operate">
            <el-button type="warning" size="mini" @click="operateForm(child, '编辑', 'Update')">编辑</el-button>
            <el-button v-show="child.belong === '000000000000000000000000'" type="primary" size="mini" @click="operateForm(child._id, '添加', 'AddSon')">添加子类</el-button>
