<template>
  <el-dialog :visible.sync="isShow" title="餐食产品" width="1000px" @closed="closed" :close-on-press-escape="false" :close-on-click-modal="false">
...
<script>
import { GetAllCategory, getVShopMakeProduct } from '@/api/system'
  ...
  methods: {
    ...
    savedData() {
      if (this.selectedProduct.length > 0) {
        this.$emit('savedMealData', this.selectedProduct, this.cateId, this.type)
        this.closed()
      } else {
        this.$message({
          message: '至少选中一条数据',
    ...
    closed() {
      this.$emit('closedMealDialog', false)
