<template>
  <el-dialog :visible.sync="isShow" title="餐食原料产品选择" width="1000px" @closed="closed" :close-on-click-modal="false">
...
<script>
import { getRawProduct, GetAllCategory, getFoodDrinkCost } from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    saveSelectedInfo() {
      this.$emit('saveFoodSelected', this.selectInfo)
      // this.closed()
    },
    searchCondition() {
      this.pageIndex = 1
    ...
    closed() {
      this.$emit('handleFoodRawSelect', false)
    }
  }
}
</script>
