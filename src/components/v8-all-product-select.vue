  ...
  <el-dialog :visible.sync="isShow" title="总库商品选择器" width="1000px" @closed="closed" :close-on-press-escape="false" :close-on-click-modal="false">
    ...
    <el-form class="filters" ref="searchForm" :model="search" :inline="true">
      <div class="filters-left">
        <el-form-item prop="categorys"> <el-cascader v-model="search.categorys" :props="casProps" :options="categoryList" clearable placeholder="请选择产品分类"></el-cascader> </el-form-item>
        <el-form-item prop="productName"> <el-input v-model="search.productName" placeholder="请输入检索内容" class="filters-search" @keyup.enter.native="GetListByPage"></el-input> </el-form-item>
        <el-form-item prop="barCode"> <el-input v-model="search.barCode" placeholder="请输入商品条码" class="filters-search" @keyup.enter.native="GetListByPage"></el-input> </el-form-item>
    ...
    <el-table class="table-main" ref="listTable" size="small" :row-key="getRowKeys" :data="productList" border @selection-change="selectTableRow" v-loading="listLoading">
      ...
      <el-table-column align="center" prop="categorysName" label="产品类别" min-width="140" show-overflow-tooltip></el-table-column>
      <el-table-column align="center" prop="stock" label="库存" min-width="100"></el-table-column>
      <el-table-column align="center" prop="unitName" label="主单位" min-width="70" show-overflow-tooltip></el-table-column>
      <el-table-column prop="barCode" label="产品条码" min-width="160" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <div v-for="(item, index) of scope.row.sku" :key="index">{{ item.barCode + (index + 1 !== scope.row.sku.length ? ';' : '') }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="供货价" min-width="90" show-overflow-tooltip>
        <template slot-scope="scope">
          <div v-for="(item, index) of scope.row.sku" :key="index">{{ item.costPrice + '元' + (index + 1 !== scope.row.sku.length ? ';' : '') }}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="市场参考价" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
